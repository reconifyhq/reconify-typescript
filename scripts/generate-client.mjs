...
for (const [path, pathItem] of Object.entries(spec.paths)) {
  for (const [method, operation] of Object.entries(pathItem)) {
    ...
    const moduleConfig = moduleNames[tag];
    if (!moduleConfig) throw new Error(`Unsupported public OpenAPI tag: ${tag}`);
    const [module, className] = moduleConfig;
    operations.push({
...
const generatedApiFiles = new Set([...moduleSources.keys()].map((module) => `${module}.ts`));
for (const entry of fs.readdirSync("src/apis", { withFileTypes: true })) {
  if (entry.isFile() && entry.name.endsWith(".ts") && !generatedApiFiles.has(entry.name)) {
    fs.rmSync(`src/apis/${entry.name}`);
  }
}
...
console.log(`Generated ${operations.length} public operations.`);
